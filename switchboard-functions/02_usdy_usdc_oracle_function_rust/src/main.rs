...
pub async fn etherprices_oracle_function(
    ...
    let etherprices = EtherPrices::fetch(
        ...
        ethers::types::U256::from(ToPrimitive::to_u128(&usdy_mean).unwrap_or(return Err(
            Error::InvalidResult,
        )?)),
        ethers::types::U256::from(ToPrimitive::to_u128(&usdy_median).unwrap_or(return Err(
            Error::InvalidResult,
        )?)),
        ethers::types::U256::from(ToPrimitive::to_u128(&population_std).unwrap_or(return Err(
            Error::InvalidResult,
        )?)),
    )
    .await
